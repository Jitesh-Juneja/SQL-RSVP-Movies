...
-- Type your code below:
select  sum(title is null) as title_count_null,
        sum(year is null) as year_count_null,
        sum(date_published is null) as date_published_count_null,
        sum(duration is null) as duration_count_null,
        sum(country is null) as country_count_null,
        sum(worlwide_gross_income is null) as worlwide_gross_income_count_null,
        sum(languages is null) as languages_count_null,
        sum(production_company is null) as production_company_count_null
from movie;
...
with single_genre_movie_production as (
  ...
  from  single_genre_movie_production; 
...
with genre_move_count_ranking as(
        ...
        from genre        
        group by genre
)select * from genre_move_count_ranking
...
with movie_avg_rating_raking as (
        ...
        from movie m
			 ...
			 on
			 m.id = r.movie_id
)SELECT * from movie_avg_rating_raking
 where movie_rank <= 10;
...
) select * from movie_production_ranking
...
-- Type your code below:
select sum(case when m.languages like '%German%' then r.total_votes else 0 end) as german_total_votes,
...
(
                ...
                select genre
                from genre_movie_count_ranking
                where genre_rank <= 3         
                
        ),
        director_movie_count_ranking as
        (
...
select director_name,
         movie_count
  from director_movie_count_ranking
...
with actor_movie_count_ranking as (
        ...
        movie_count
 from actor_movie_count_ranking
 where actor_rank <= 2;
...
)select * from production_company_votes_count_ranking
...
)select * from actress_avg_rating_ranking
...
) select * from multilingual_movies_count_ranking
